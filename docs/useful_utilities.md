...
## App Launcher: Wofi 🚀
[Wofi](https://github.com/philj56/Wofi) offers a minimalistic, Wayland-native application launcher for efficient application access.
...
`~/.config/wofi/` - The configuration directory.

You can copy my pre-configured files:

```
cp -r ~/simple-hyprland/configs/wofi ~/.config/
```
...
```
$menu = wofi-drun -c ~/.config/wofi/configA --drun-launch=true
```
